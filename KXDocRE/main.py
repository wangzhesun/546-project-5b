...
explaindata = list
contextdata = dict()
with open('/home/ecrim/ecrim/context_aware_entitytype_with_context.json', 'r') as file1:
...
def eval_performance(facts, pred_result):
    ...
    if wand_b:
        wandb.log({"mean_precision": mean_prec, "f1_score": f1, "auc_score": auc})
...
def process(tokenizer, h, t, doc0, doc1, ke):
    ht_markers = ["[unused" + str(i) + "]" for i in range(1, 5)]
    b_markers = ["[unused" + str(i) + "]" for i in range(5, 101)]
    # context_markers=['[unused10]', '[unused11]', '[unused12]', '[unused13]']
    ...
    for blk in dbuf:
        # print("HT mARKERS:",ht_markers)
        # print("BLOCK DETAILS:",blk)
...
def find_context_span(doc_tokens, context_path):
    # Function to find the span position of context_path in a document
    # Assuming context_path is a list of tokens
    span_start = None
    span_end = None

    for i in range(len(doc_tokens) - len(context_path) + 1):
        if doc_tokens[i:i + len(context_path)] == context_path:
            span_start = i
            span_end = i + len(context_path) - 1
            break

    return span_start, span_end


def process_example_simp_context(h, t, doc1, doc2, tokenizer, max_len, redisd, no_additional_marker, mask_entity):
    max_len = 99999
    bert_max_len = 512
    contextpath1 = ""
    contextpath2 = ""
...
def collate_fn(batch, args, relation2id, tokenizer, redisd, encoder, sbert_wk):
    ...
    if batch[0][-1] == 'o':
        ...
        for doc1, doc2, l in dps:
            ...
            selected_ret = process_example_simp_context(h, t, doc1, doc2, tokenizer, args.seq_len, redisd,
        ...
        for idx, example in enumerate(examples):
            ...
            hid = ""
            tid = ""
            if str(h_name) in entity2id:
                hid = entity2id[str(h_name)]
            if str(t_name) in entity2id:
        ...
        for pi in range(input_ids.size()[0]):
            ...
            if len(b_spans) > 0 and len(b_spans) % 2 == 0:
                ...
                b_mask = torch.zeros_like(input_ids[pi]).to(input_ids.device).scatter(
                    0, torch.tensor(b_span_, dtype=torch.int64).to(input_ids.device), 1
                )
            ...
            htb_mask = torch.cat([h_mask.unsqueeze(0), t_mask.unsqueeze(0), b_mask.unsqueeze(0)], dim=0)  # [3,512]
...
class CodredCallback(TrainerCallback):
    ...
    def on_dev_step(self, step, inputs, extra, outputs):
        ...
        r_score_cpu = r_score.cpu()  # Transfer CUDA tensor to CPU
        ht_threshold_cpu = ht_threshold_flatten[:, 0, 0].cpu()  # Transfer CUDA tensor to CPU if needed

        if torch.gt(r_score_cpu, ht_threshold_cpu).any():
            # Your code logic here
            # if np.greater(r_score, ht_threshold_flatten[:, 0, 0]).any():
